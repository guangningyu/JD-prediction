...
if __name__ == '__main__':
    ...
    # step_01 读入数据集
    master_table_address = r'C:\shu.wen\opera\00_projects\99_competition\01_JData\data\rule\RULE_USER_CATE_ACTION_MST_TBL_v4.csv'
    ...
    print 'row_cnt: %d, trt_rt: %.4f'%(master_df_filter.shape[0], master_df_filter.Y.sum()*1.0/master_df_filter.shape[0])
    ...
    P1_1_train = precision_score(master_df['Y'], master_df['y_pre'])
    R1_1_train = recall_score(master_df['Y'], master_df['y_pre'])
    F11_train = 6*R1_1_train*P1_1_train/(5*R1_1_train+P1_1_train)
    print
    print 'F11: %.4f, P: %.4f, R: %.4f'%(F11_train, P1_1_train, R1_1_train)


    # step_01 读入数据集
    master_table_address = r'C:\shu.wen\opera\00_projects\99_competition\01_JData\data\rule\RULE_USER_CATE_ACTION_MST_TBL_v4.csv'
    master_df[master_df['y_pre'] == 1][['USER_ID', 'USER_SKU_DIS_CNT']].to_csv(r'C:\shu.wen\opera\00_projects\99_competition\01_JData\data\rule\USER_ID_CANDIDATE.csv', sep=',', index=False )
